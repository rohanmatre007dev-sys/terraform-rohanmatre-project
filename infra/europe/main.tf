...
terraform {
  ...
  backend "s3" {
    ...
    region         = "eu-west-1"
    dynamodb_table = "terraform-lock-europe"
    encrypt        = true
  }
}
...
module "sns" {
  ...
  display_name = "Healthcare Emergency Alerts - Europe"
...
module "cw_log_monitoring" {
  ...
  log_group_name              = "/healthcare/europe/patient-monitoring"
...
module "cw_log_alert" {
  ...
  version                     = "~> 1.0"
  create_log_group            = true
  log_group_name              = "/healthcare/europe/emergency-alert"
  log_group_retention_in_days = 30
}
...
module "cw_log_data" {
  ...
  version                     = "~> 1.0"
  create_log_group            = true
  log_group_name              = "/healthcare/europe/patient-data"
...
resource "aws_secretsmanager_secret" "db_credentials" {
  ...
  description             = "RDS credentials for Europe region"
